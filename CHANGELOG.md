...
- DEPENDENCY VERSION SWEEP: NPM, CARGO, AND AI SDK PACKAGES UPDATED; GITHUB ACTIONS BUMPED.
- CLIENT JS SIZE BUDGET RAISED TO ACCOMMODATE THE NEW LSP FRONTEND.
- FIRST COMPONENT-TEST STACK (HAPPY-DOM + TESTING-LIBRARY/REACT); ALL TESTS CONSOLIDATED INTO A ROOT `tests/` FOLDER.
- NEW `scripts/bump-version.mjs` KEEPS EVERY VERSION-BEARING MANIFEST IN SYNC ON EACH RELEASE (SEE CONTRIBUTING.MD).
...
- UPGRADED THE TYPESCRIPT TOOLCHAIN TO THE NATIVE 7.0 COMPILER FOR FASTER TYPE-CHECKS.
...
- CLI_CK_OPEN SHELL COMMAND - OPEN FILES INTO A cli-ck EDITOR TAB FROM ANY TERMINAL.
