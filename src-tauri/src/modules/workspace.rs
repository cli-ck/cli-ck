...
pub fn wsl_path_to_unc(distro: &str, path: &str) -> PathBuf {
    ...
    if !is_safe_distro_name(distro) {
        return PathBuf::from(r"\\wsl.localhost\__cli_ck_invalid_distro__");
    }
    let normalized = path.replace('\\', "/");
...
mod tests {
    ...
    fn wsl_path_to_unc_blocks_traversal_distro() {
        ...
        assert!(s.contains("__cli_ck_invalid_distro__"), "got: {s}");
...
mod auth_tests {
    ...
    fn tempdir(label: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("cli-ck-auth-{label}-{nanos}-{}", std::process::id()));
    ...
    fn authorize_user_spawn_cwd_rejects_missing_path() {
        let mut missing = env::temp_dir();
        missing.push(format!("cli-ck-user-missing-{}", std::process::id()));
        let reg = WorkspaceRegistry::default();
        let s = missing.to_string_lossy().into_owned();
    ...
    fn user_spawn_cwd_or_home_falls_back_when_inaccessible() {
        let mut missing = env::temp_dir();
        missing.push(format!("cli-ck-orhome-missing-{}", std::process::id()));
        let reg = WorkspaceRegistry::default();
        let s = missing.to_string_lossy().into_owned();
        assert_eq!(
            user_spawn_cwd_or_home(&reg, Some(&s), &WorkspaceEnv::Local),
    ...
    fn resolve_launch_cwd_ignores_nonexistent_cli_dir() {
        ...
        let resolved = resolve_launch_cwd(Some("/no/such/cli-ck/dir"), Some(env.clone()));
...
mod appimage_tests {
    ...
    fn strips_appdir_from_path_lists_and_unsets_when_empty() {
        let appdir = Path::new("/tmp/.mount_cli-ck_X");
        let env = reader(&[
            ("LD_LIBRARY_PATH", "/tmp/.mount_cli-ck_X/usr/lib:/usr/lib"),
            ("PATH", "/tmp/.mount_cli-ck_X/usr/bin:/usr/bin:/bin"),
            ("GST_PLUGIN_SYSTEM_PATH", "/tmp/.mount_cli-ck_X/usr/lib/gstreamer-1.0"),
            ("APPDIR", "/tmp/.mount_cli-ck_X"),
        ]);
    ...
    fn unsets_value_vars_only_when_pointing_into_appdir() {
        let appdir = Path::new("/tmp/.mount_cli-ck_X");
        let into = reader(&[("LD_PRELOAD", "/tmp/.mount_cli-ck_X/usr/lib/x.so")]);
