...
const OWNED_MARKERS: [&str; 2] = ["notify;cli-ck;", "cli-ck;notify"];

// Gated on CLI_CK_TERMINAL; no-op outside cli-ck. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$CLI_CK_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;cli-ck;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.cli-ck-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;cli-ck;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;cli-ck;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;cli-ck;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;cli-ck;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$CLI_CK_TERMINAL\" ] && printf '\\033]777;cli-ck;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
