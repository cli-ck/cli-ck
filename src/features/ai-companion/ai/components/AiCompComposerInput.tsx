...
import { useComposer } from "../lib/aiComposer";
...
export function AiCompComposerInput() {
  const c = useComposer();
  const snippets = useSnippetsStore((s) => s.snippets);
  const workspaceRoot = useAiChatStore((s) => s.live.getWorkspaceRoot());
  ...
  return (
    <>
        ...
        <PopoverAnchor asChild>
          <div className="flex items-start gap-2">
            <textarea
              ...
              placeholder="Ask cli-ck anything   -   # for snippets and commands, @ for files"
              rows={1}
              className={cn(
      ...
      {voiceRow.mounted && (
        <div data-state={voiceRow.state} className="cli-ck-reveal">
