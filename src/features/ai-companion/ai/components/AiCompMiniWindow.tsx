...
import { getOrCreateChat } from "../store/aiChatRuntime";
...
export function AiCompMiniWindow({ state }: { state: PresenceState }) {
  const closeMini = useAiChatStore((s) => s.closeMini);
...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-8 py-10 text-center">
      <CliCkLogo size={56} className="opacity-90" />
      <div className="space-y-1.5">
        <p className="text-[14px] font-semibold tracking-tight">
          Ask cli-ck anything
        </p>
        <p className="max-w-[18rem] text-[11.5px] leading-relaxed text-muted-foreground">
          cli-ck sees the active terminal — cwd, recent commands, and output.
        </p>
      </div>
