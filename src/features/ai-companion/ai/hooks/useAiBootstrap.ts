...
import { firePendingReviewForSession } from "@/features/ai-companion/agents/lib/review";
...
export function useAiBootstrap(): {
  ...
  const setApiKeys = useAiChatStore((s) => s.setApiKeys);
  const setCustomEndpointKeys = useAiChatStore((s) => s.setCustomEndpointKeys);
  const setSelectedModelId = useAiChatStore((s) => s.setSelectedModelId);
  const activeSessionId = useAiChatStore((s) => s.activeSessionId);
  const hydrateSessions = useAiChatStore((s) => s.hydrateSessions);
  ...
  // into aiChatStore so the dropdown reflects what the user picked in Settings.
