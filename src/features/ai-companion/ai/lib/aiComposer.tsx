...
import { currentWorkspaceEnv } from "@/features/shell-pty/workspace";
...
export function AiComposerProvider({ children }: ProviderProps) {
  const sessionId = useAiChatStore((s) => s.activeSessionId);
  const status = useAiChatStore((s) => s.agentMeta.status);
  ...
  const focusSignal = useAiChatStore((s) => s.focusSignal);
  const pendingPrefill = useAiChatStore((s) => s.pendingPrefill);
  const consumePrefill = useAiChatStore((s) => s.consumePrefill);
  const pendingSelections = useAiChatStore((s) => s.pendingSelections);
  const consumeSelections = useAiChatStore((s) => s.consumeSelections);
  ...
  useEffect(() => {
    ...
    window.addEventListener("cli-ck:ai-attach-file", onAttach);
    return () => window.removeEventListener("cli-ck:ai-attach-file", onAttach);
  ...
  const attachFileByPath = async (path: string) => {
    try {
      ...
      useAiChatStore.getState().focusInput();
    } catch (e) {
  ...
  const submit = () => {
    ...
    // the prompt to the cli-ck.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<cli-ck-command name="${outcome.commandName}" />`;
        }
      }
    }
